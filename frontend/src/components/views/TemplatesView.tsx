...
export const TemplatesView: React.FC<TemplatesViewProps> = ({ projects = [], onNavigate, currentPersona }) => {
  ...
  // Use Template State
  const [activeTemplateForUse, setActiveTemplateForUse] = useState<ExecutiveTemplate | null>(null);
  const [formFieldValues, setFormFieldValues] = useState<Record<string, any>>({});
  const [applicantName, setApplicantName] = useState(currentPersona?.name || 'Project Manager');

  // Decision State
  const [activeRequestForDecision, setActiveRequestForDecision] = useState<ExecutiveTemplateRequest | null>(null);
  const [executiveNameInput, setExecutiveNameInput] = useState(currentPersona?.name || 'Sarah Jenkins (PMO Director)');
  ...
  const handleSaveVersionBump = (e: React.FormEvent) => {
    ...
    const newVerNumber = parseFloat(bumpVersionNumber) || 1.1;
